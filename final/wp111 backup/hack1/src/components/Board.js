        ...
        <div className="Board-container">
                ...
                Hint: Use `CurRow` instead of `Row` when you are passing `curGuess` into it. */}
            {
                guesses.map((guess, wordIndex) => {
                    let idName = "row_" + toString(wordIndex);
                    if (wordIndex !== turn)
                        return (<Row id={idName} key={idName} guess={guess} rowIdx={wordIndex} />);
                    else
                        return (<CurRow id={idName} key={idName} curGuess={curGuess} rowIdx={wordIndex} />);
                })
            }
        </div>
    )
